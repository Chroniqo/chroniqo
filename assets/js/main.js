...
// Add-to-cart buttons from all sections
const addToCartButtons = document.querySelectorAll(
    '.home__button, .featured__button, .products__button, .new__button'
)
...
addToCartButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        const title = card.querySelector('h3, h1')?.innerText || 'Unnamed Product'
        const price = card.querySelector('span[class*="price"]')?.innerText || '$0'
...
function updateCart() {
        ...
        cartItem.innerHTML = `
            ...
            <div class="cart__item-details">
                ...
                <div class="cart__quantity-controls">
                    ...
                    <span class="cart__qty-number">${item.quantity}</span>
                    <button class="cart__qty-btn" data-action="increase" data-index="${index}">+</button>
                </div>
            </div>
            <button class="cart__remove" data-index="${index}">REMOVE</button>
        `;
    ...
    document.querySelectorAll('.cart__qty-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const i = btn.dataset.index;
            ...
            } else if (action === 'decrease' && cartItems[i].quantity > 1) {
                cartItems[i].quantity--;
            }
            updateCart();
        });
